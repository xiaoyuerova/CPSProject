...
from tcn_test_8_3.data_tcn.MyDataset import MyDataset
...
class Data:
    ...
    def collate_batch(self, batch):
        ...
        """
        label_batches, text_batches, offset_batches, select_batches = [], [], [], []
        for idx, (labels, texts, select) in enumerate(batch):
            label_batches.append(self.label_pipeline(labels))
            select_batches.append(select)
        ...
        select_batches = torch.tensor(select_batches, dtype=torch.int64)
        return label_batches.to(self.device), text_batches.to(self.device), offset_batches.to(
            self.device), select_batches.to(self.device)
