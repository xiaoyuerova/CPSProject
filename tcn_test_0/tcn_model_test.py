...
from tcn_test_0.data_tcn import *
from tcn_test_0.data_tcn.parameters import Parameters
from tcn_test_0.model import TextClassification
from tcn_test_0.utils import build_vocab_from_iterator_re, data_iter
...
# 准备模型
model = TextClassification(vocab_size, 11)
...
def evaluate(data: Data, epoch: int):
    ...
    model.eval()
    for idx, (label, text, offsets) in enumerate(data.dataloader):
        optimizer.zero_grad()
...
def train(train_data: Data, epoch: int):
    ...
    start_time = time.time()
    model.train()
    for idx, (label, text, offsets) in enumerate(train_data.dataloader):
        optimizer.zero_grad()
        output = model(text, offsets)
