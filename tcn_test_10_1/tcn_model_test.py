...
from tcn_test_10_1.data_tcn import *
from tcn_test_10_1.data_tcn.parameters import Parameters
from tcn_test_10_1.model import CpsTcnModel
from tcn_test_10_1.utils import build_vocab_from_iterator_re, data_iter, pre_process
...
def evaluate(data: Data, epoch: int):
    ...
    model.eval()
    for idx, (label, action, mask) in enumerate(data.dataloader):
        optimizer.zero_grad()
        output = model(action)
        out_temp = []
        for index, flag in enumerate(mask[0]):
            if flag:
                out_temp.append(output[index])
        output = torch.cat([item.view(1, -1) for item in out_temp])
...
def train(train_data: Data, epoch: int):
    ...
    start_time = time.time()
    model.train()
    for idx, (label, action, mask) in enumerate(train_data.dataloader):
        # print('mask_list', mask[0].size())
        optimizer.zero_grad()
        output = model(action)
        out_temp = []
        for index, flag in enumerate(mask[0]):
        ...
        for j in output.argmax(1).eq(label[0]):
            if j:
                correct += 1
