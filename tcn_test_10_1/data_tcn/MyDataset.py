    ...
    def __getitem__(self, index):
        ...
        for index in group.index:
            if group.at[index, 'DataCode'] == 5000:
                ...
                mask.append(0)
        # print('my dataset', sum([1 if m == 0 else 0 for m in mask]))
        for index in group.index:
            group.at[index, 'Action'] = int(group.at[index, 'Action'])
        actions = group['Action'].to_list()
        group = group[group['DataCode'] == 5000]
        labels = group['Label'].to_list()
        # print('my dataset', len(labels))
        return labels, actions, mask
