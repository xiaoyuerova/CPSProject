...
class Data:
    ...
    def collate_batch(self, batch):
        label_list, action_list, mask_list = [], [], []
        for (labels, actions, mask) in batch:
            label_list.append(self.label_pipeline(labels))
            action_list.append(self.action_pipeline(actions))
        ...
        masks = torch.tensor(mask_list, dtype=torch.int64).to(parameters.device)
        return labels, actions, masks
